...
func showMessages(w http.ResponseWriter, r *http.Request) {
	...
	qSlice := make([]Message, 0)
	// если в query запросе есть ключ postcode
	if r.URL.Query().Has("postcode") {
		qString := r.URL.Query().Get("postcode")
		qInt, err := strconv.Atoi(qString)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			log.Println("пользователь ввел неверный query запрос с ключем postcode")
			return
		}
		mtx.RLock()
		for _, v := range allMessages {
		...
		mtx.RUnlock()
	} else if r.URL.Query().Has("urgent") {
		// если в query запросе есть ключ urgent
		qString := r.URL.Query().Get("urgent")
		qBool, err := strconv.ParseBool(qString)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			log.Println("пользователь ввел неверный query запрос с ключем urgent")
			return
		}
		mtx.RLock()
		for _, v := range allMessages {
		...
		// если тело query запроса пустое
		mtx.RLock()
		if err := json.NewEncoder(w).Encode(allMessages); err != nil {
			log.Println("ошибка в чтении json:", err)
			return
		...
		w.WriteHeader(http.StatusBadRequest)
		log.Println("пользователь ввел несуществующий query запрос")
		return
	}

	mtx.RLock()
	if err := json.NewEncoder(w).Encode(qSlice); err != nil {
		log.Println("ошибка в чтении json:", err)
		return
	}
	mtx.RUnlock()
